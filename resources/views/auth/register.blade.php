    ...
    <div class="min-h-screen flex items-center justify-center p-4">
        <div class="w-full max-w-6xl bg-slate-800/50 backdrop-blur-xl rounded-3xl shadow-2xl border border-slate-700/50 overflow-hidden">
            <div class="grid lg:grid-cols-5">
                    ...
                    <div class="space-y-4 relative z-10">
                        <div class="flex items-start gap-3 bg-white/10 backdrop-blur-sm rounded-xl p-3">
                            <div class="p-2 bg-blue-500 rounded-lg shrink-0">
                                <span class="iconify" data-icon="lucide:zap" style="width: 18px; height: 18px;"></span>
                            </div>
                            <div>
                                <div class="font-semibold text-sm">Rapide</div>
                                <div class="text-xs text-blue-100">Prêt en 5 minutes</div>
                            </div>
                        </div>
                        <div class="flex items-start gap-3 bg-white/10 backdrop-blur-sm rounded-xl p-3">
                            <div class="p-2 bg-blue-500 rounded-lg shrink-0">
                ...
                <!-- Right Side - Register Form (3 cols) -->
                <div class="lg:col-span-3 p-8 bg-slate-800">
                    <div class="max-w-md mx-auto">
                        <div class="mb-6">
                            <h2 class="text-2xl font-bold tracking-tight text-white mb-1">Créer un compte</h2>
                            <p class="text-slate-400 text-sm">Essai gratuit de 7 jours</p>
                        </div>
                        ...
                        <form method="POST" action="{{ route('register') }}" class="space-y-3">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    <label class="block text-xs font-medium text-slate-300 mb-1.5">Nom complet</label>
                                    <input type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name" placeholder="Jean Dupont" class="w-full px-3 py-2.5 bg-slate-700/50 border border-slate-600 rounded-xl text-sm text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                            ...
                            <div class="grid grid-cols-2 gap-3">
                                <div>
                                    ...
                                    <input type="password" name="password" required autocomplete="new-password" placeholder="Min. 8 caractères" class="w-full px-3 py-2.5 bg-slate-700/50 border border-slate-600 rounded-xl text-sm text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                                    @error('password')<span class="text-red-400 text-xs mt-1">{{ $message }}</span>@enderror
                                </div>

                                <div>
                                    <label class="block text-xs font-medium text-slate-300 mb-1.5">Confirmer</label>
                                    <input type="password" name="password_confirmation" required autocomplete="new-password" placeholder="Retapez" class="w-full px-3 py-2.5 bg-slate-700/50 border border-slate-600 rounded-xl text-sm text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                                </div>
                            </div>

                            <div class="p-3 bg-blue-500/10 border border-blue-500/30 rounded-xl">
                                <div class="flex items-start gap-2">
                                    <span class="iconify text-blue-400 mt-0.5 shrink-0" data-icon="lucide:info" style="width: 16px; height: 16px;"></span>
                                    <p class="text-xs text-blue-300">
                                        Après inscription, configurez votre organisation et commencez immédiatement.
                                    </p>
                                </div>
                            </div>
                            ...
                            <div class="text-center text-xs text-slate-400 pt-2">
                                Déjà un compte ? <a href="{{ route('login') }}" class="text-blue-400 hover:text-blue-300 font-semibold">Se connecter</a>
                            </div>
                        </form>
                    </div>
                </div>
